...
def test_delete_matching_success(client, db_session, monkeypatch):
    ...
    u1 = User(
        name="테스터1", gender="MALE", birth_year=1990, job="직업1",
        contact="010-0000-0001", password_hash="hash", referrer_name="지인1",
        desired_conditions="조건1", deal_breakers="기피1"
    )
    u2 = User(
        name="테스터2", gender="FEMALE", birth_year=1992, job="직업2",
        contact="010-0000-0002", password_hash="hash", referrer_name="지인2",
        desired_conditions="조건2", deal_breakers="기피2"
    )
    db_session.add(u1)
    db_session.add(u2)
    db_session.commit()
    ...
    deleted = db_session.query(Matching).filter(Matching.id == matching_id).first()
...
def test_delete_matching_not_found(client, monkeypatch):
    ...
    response = client.delete("/api/matchings/00000000-0000-0000-0000-000000000000", headers=headers)
