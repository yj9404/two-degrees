...
def get_ai_recommendations(target_user: Dict[str, Any], candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    system_instruction = (
        ...
        "4. **가치관 항목 분석**: 데이터에 포함된 marriage_intent(결혼 의향)와 child_plan(자녀 계획) 항목을 분석하십시오. "
        "두 사람의 가치관이 서로 일치하거나 상호 보완적이면 점수에 가산점을 부여하십시오. "
        "반대로 가치관이 명백히 상충하는 경우(예: 한 분은 NON_MARRIAGE이고 다른 분은 WILLING)에는 적합도 점수를 크게 낮추십시오.\n"
        "5. **UNKNOWN 처리**: marriage_intent 또는 child_plan 값이 'UNKNOWN'인 경우, "
        "해당 항목 미등록으로 인해 불이익을 주지 마십시오. "
        "대신 추천 사유(reason)에 '아직 가치관 정보가 등록되지 않은 부분이 있어, 대화를 나누시며 서로의 미래에 대한 생각을 천천히 알아가시기를 권해 드립니다'와 같은 내용을 자연스럽게 포함하십시오.\n"
        "6. **완곡한 표현 유지**: 가치관 일치 여부를 언급할 때 '결혼하고 싶어 함', '아이를 원함' 같은 직설적 표현 대신 "
        "'미래에 대한 지향점이 비슷하여 깊은 대화를 나누기에 좋은 짝입니다', '삶의 방향성이 자연스럽게 맞닿아 있어 편안하게 미래를 그려갈 수 있을 것 같습니다'와 같이 주선자 톤을 유지하십시오.\n"
        "7. **사내연애 방지(직장 처리)**: 타겟 유저와 후보 유저의 직장(workplace/company)이 서로 동일하더라도 절대 가산점을 부여하지 마십시오. 오히려 직종(job)이나 업계는 유사하되 서로 다른 회사에 소속된 경우를 더 높은 적합도로 판단하십시오.\n"
        "8. **MBTI 처리**: MBTI가 서로 같거나 일반적인 엠비티아이 궁합이 좋다는 이유만으로 가산점을 부여하지 마십시오. 단, 유저가 작성한 원하는 조건(desired_conditions)이나 기피 조건(deal_breakers)에 특정 MBTI 성향을 명시한 경우(예: 'T 성향은 피하고 싶어요')에는 이를 점수 및 평가에 적극 반영하십시오."
    )
